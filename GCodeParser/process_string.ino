...
void ParseGCodeInstruction( char * instruction, byte & index , byte size )
{
  ...
  switch( Command )
  {
    ...
    case 1:
      ...
      // JCW the max feedrate thing below just doesn't seem to work at the moment.
      if (Command == 1)
      {
        //how fast do we move?
        if( gc.paramValid & P_F ) {
          ...
          feedrate_micros = calculate_feedrate_delay(feedrate);
           if (DEBUG == 1){
            Serial.print("feedrate_micros is ");
    ...
    case 28:
      set_ftarget(0, 0, 0);
      //JCW max speed doesn't work at the moment. change to feedrate code
        if (DEBUG == 1){
          Serial.print("getMaxSpeed is ");
          Serial.println(getMaxSpeed());
        }
        //JCW - set the rapid byte from 1 to 0...
      dda_move(getMaxSpeed(), 0);
...
void ParseMCodeInstruction( char * instruction, byte & index , byte size )
{
  ...
  //Serial.print( "M" );
  //Serial.println(Command, DEC);
  // JCW faking out the M commands...  repetier is unhappy if you don't give it a temperature...
  
  if (Command == 105){
    Serial.println("ok T:205");
  } else {
    Serial.println("ok");
  }
