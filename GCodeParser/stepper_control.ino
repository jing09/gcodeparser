...
// and subsequently modified by John Weers - some of the speed stuff isn't working with my steppers
...
void set_laser_power(void)
{
  ...
  //if (DEBUG == 1){
  //  Serial.print("Setting Laser Power: ");
  //  Serial.println(lz);
  //}
...
void dda_move(long micro_delay , byte rapid)
{
  ...
  long curStep = 1; // used only during rapid moves, for accel/decel tracking
    ...
    // JCW - adjust to put back the pen function.   
    // it should check and modify the laser if the laser is being used (settable by gcode)
    // or use the pen if it's being used (also settable by gcode)
    // with the PEN, Z must move first.
    // with the laser, they should happen about the same time and it's good enough.
    // the pen just takes a lot longer to move...
    if (z_can_step)
    {
      z_counter += delta_steps.z;
      if ((Z_ENABLE_SERVO == 1) && (Z_ENABLE_LASER==0)){
        if (target_steps.z < 0){
          servo.write(90);
          delay(1);
        } 
